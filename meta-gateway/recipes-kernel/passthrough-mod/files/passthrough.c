...
/*static int passthrough_irq_setup(struct device_node *np, struct passthrough_instance *HLS_IP)
{
	u32 irq;
	struct resource res;
	...
	if (!ret) {
		pr_info("Passthrough IRQ not connected\n");
		return 0;
	}

	//chip->mmchip.gc.to_irq = xgpio_to_irq;

	//chip->irq_base = irq_alloc_descs(-1, 0, 1, node);
	HLS_IP->irq_base = irq_alloc_desc((int)np);
	if (HLS_IP->irq_base < 0) {
		pr_err("Couldn't allocate IRQ numbers\n");
		return -1;
	}
	//Prototype: irq_domain_add_legacy(struct device_node *of_node, 
	//unsigned int size, unsigned int first_irq, irq_hw_number_t first_hwirq, 
	//const struct irq_domain_ops *ops, void *host_data);
	HLS_IP->irq_domain = irq_domain_add_legacy(np, 
						1,
						 HLS_IP->irq_base, 0,
	...
	irq = irq_find_mapping(HLS_IP->irq_domain, 0);
	irq_set_lockdep_class(irq, &hls_ip_lock_class);
	pr_debug("IRQ Base: %d, IRQ %d\n", HLS_IP->irq_base, irq);
	//irq_set_handler(unsigned int irq, irq_flow_handler_t handle);
	//irq_set_handler_data(res.start, (void *)HLS_IP);
	//irq_set_chained_handler(res.start, xgpio_irqhandler);
